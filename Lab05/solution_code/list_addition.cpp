 ...
 * Challenge Exercise
 */
std::list<int> add_lists(std::list<int> const& list1, std::list<int> const& list2) {
	std::list<int> sum {};
	bool carry {false};
	...
	for (; it1 != list1.rend() && it2 != list2.rend(); ++it1, ++it2) {
		// sum the pair digits together
		int value {*it1 + *it2};

		// if the previous iteration produced a carry, add it to value
		if (carry) {
			++value;
			carry = false;
		}

		// check if a carry occurred in this iteration
		if (value / 10 != 0) {
			carry = true;
			value -= 10;
		}

		// push value into output sum list
		sum.push_front(value);
	}
	
	// handle case where final digit sum produced a carry
	if (carry) {
		sum.push_front(1);
	}
